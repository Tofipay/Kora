/* ALOKA Live — frontend runtime (no framework, ~8KB gzipped)
 *
 * Structure:
...
(function () {
  'use strict';
  const Q = window.ALOKA || { lang: 'ar', prefix: '', t: {}, fcm: {} };
  ...
  function applyTheme(mode) {
    ...
    root.style.backgroundColor = dark ? '#1b0761' : '#ffffff';
   ...
   * dialogs never stack. */
  (function () {
    const TG_URL = 'https://t.me/alokalive';
    const TKEY = 'q_tg';
   ...
   * channels, matches) swaps to the ALOKA Live mark — never a broken icon.
   * Capture-phase listener = works for every current and future image. */
  const IMG_FALLBACK = '/assets/brand/icon.svg';
  function prepareImage(img) {
    if (!(img instanceof HTMLImageElement)) return;
    img.classList.toggle('image-ready', !!(img.complete && img.naturalWidth > 0));
  }
  addEventListener('load', e => {
    if (e.target instanceof HTMLImageElement) e.target.classList.add('image-ready');
  }, true);
   ...
   * kinds: team / league / match (sports) + movie / series + channel. */
  ...
  window.QF = {
    toggle(btn) {
      ...
      if (f[kind][id]) { delete f[kind][id]; }
        ...
        f[kind][id] = { id, title: btn.getAttribute('data-title') || '', url: btn.getAttribute('data-url') || '', img: btn.getAttribute('data-img') || '', playType: btn.getAttribute('data-play-type') || '' };
        added = true;
      }
      saveFavs(f);
  ...
  /* ---------------- Android channel intents + install fallback ----------------
   * Keep the real intent:// href intact. If Android hands the request to the
   * official app, the page becomes hidden. If it remains visible, show the
   * branded install dialog instead of leaving the visitor with no feedback. */
  const appDialog = $('#app-required-dialog');
  let appAttempt = null, appDialogReturnFocus = null;
  function closeAppDialog() {
    ...
    if (appDialogReturnFocus && appDialogReturnFocus.focus) appDialogReturnFocus.focus();
  }
  function openAppDialog(downloadUrl, trigger) {
    if (!appDialog) return;
    const dl = $('#app-dialog-download', appDialog);
    if (dl && /^https?:\/\//i.test(downloadUrl || '')) dl.href = downloadUrl;
    appDialogReturnFocus = trigger || document.activeElement;
    appDialog.hidden = false;
    appDialog.setAttribute('aria-hidden', 'false');
    document.body.classList.add('dialog-open');
    requestAnimationFrame(() => {
      appDialog.classList.add('open');
      const panel = $('.app-dialog-panel', appDialog);
      if (panel) panel.focus();
    });
  }
  document.addEventListener('visibilitychange', () => {
    if (appAttempt && document.visibilityState === 'hidden') appAttempt.left = true;
  });
  addEventListener('pagehide', () => { if (appAttempt) appAttempt.left = true; });
  document.addEventListener('click', e => {
    const a = e.target.closest('a[data-channel-intent]');
    if (!a) return;
    const attempt = { left: false };
    appAttempt = attempt;
    setTimeout(() => {
      if (appAttempt === attempt && !attempt.left && document.visibilityState === 'visible') {
        openAppDialog(a.getAttribute('data-app-download') || 'https://t.me/alokalive', a);
      }
    }, 1400);
  }, true);
  document.addEventListener('click', e => {
    if (e.target.closest('[data-app-dialog-close]')) closeAppDialog();
  });
  document.addEventListener('keydown', e => {
    if (!appDialog || appDialog.hidden) return;
    if (e.key === 'Escape') { closeAppDialog(); return; }
    if (e.key !== 'Tab') return;
    const focusable = $$('a[href],button:not([disabled]),[tabindex]:not([tabindex="-1"])', appDialog).filter(el => !el.hidden);
  ...
  /* App-like content protection: suppress browser copy/share callouts and
   * long-press context menus on the public site. Inputs remain editable. */
  ...
  function initPage(scope) {
    ...
    /* Brand mark is visible inside every image box until its real image is
       decoded; failed images keep the same mark through the fallback above. */
    $$('img', scope).forEach(prepareImage);
    ...
    QF.mark(scope);
    /* Watched episodes are device-local, just like favorites. */
    let watched = {};
    try { watched = JSON.parse(localStorage.getItem('q-watched-v1') || '{}') || {}; } catch (e) {}
    $$('[data-ep-series][data-ep-season][data-ep-episode]', scope).forEach(card => {
      const key = card.dataset.epSeries + '-' + card.dataset.epSeason + '-' + card.dataset.epEpisode;
      card.classList.toggle('is-watched', !!watched[key]);
    });
    ...
    if (favRoot && !favRoot.dataset.bound) {
      ...
      const groups = [
        ...
        ['channel', favRoot.dataset.l10nChannels]
      ];
      let html = '', any = false;
      groups.forEach(([kind, label]) => {
        ...
        items.forEach(it => {
          ...
          if (poster) {
            ...
            const intentAttrs = kind === 'channel' && it.playType === 'intent' ? ' data-channel-intent data-app-download="https://t.me/alokalive"' : '';
            html += '<a class="fav-item card-hover" href="' + it.url + '"' + intentAttrs + '>' +
    ...
    /* Channels page slider: admin images, group links or ad links. */
    const channelSlider = $('[data-channel-slider]', scope);
    if (channelSlider && !channelSlider.dataset.bound) {
      channelSlider.dataset.bound = '1';
      const slides = $$('[data-channel-slide]', channelSlider);
      const dots = $$('[data-channel-goto]', channelSlider);
      const prev = $('[data-channel-prev]', channelSlider);
      const next = $('[data-channel-next]', channelSlider);
      if (slides.length > 1) {
        let index = 0, timer = 0, touchX = null;
        const show = value => {
        ...
        const play = () => {
          stop();
          timer = setInterval(() => {
            if (!channelSlider.isConnected) { stop(); return; }
            show(index + 1);
          }, 5200);
        };
        ...
        dots.forEach(dot => dot.addEventListener('click', () => { show(+(dot.dataset.channelGoto || 0)); play(); }));
        channelSlider.addEventListener('pointerdown', event => {
          if (event.pointerType === 'touch') touchX = event.clientX;
        }, { passive: true });
        channelSlider.addEventListener('pointerup', event => {
          if (touchX === null) return;
          const distance = event.clientX - touchX;
          touchX = null;
          if (Math.abs(distance) > 42) move(distance < 0 ? 1 : -1);
        }, { passive: true });
        channelSlider.addEventListener('pointercancel', () => { touchX = null; });
        channelSlider.addEventListener('pointerenter', stop);
        channelSlider.addEventListener('pointerleave', play);
        channelSlider.addEventListener('focusin', stop);
        channelSlider.addEventListener('focusout', play);
        play();
      }
    }
  }
  ...
  function updateActiveNav(pathname) {
    ...
    const aliases = { '/movies': ['/movie'], '/series': [], '/videos': ['/video'], '/matches': [], '/news': [], '/channels': [] };
...
(() => {
  ...
  document.addEventListener('click', (ev) => {
    ...
    // 3) Active episode card + local watched mark
    document.querySelectorAll('[data-ep-episode]').forEach(c => {
      const active = +c.getAttribute('data-ep-episode') === episode;
      c.classList.toggle('active', active);
      if (active) c.setAttribute('aria-current', 'true'); else c.removeAttribute('aria-current');
    });
    try {
      const watched = JSON.parse(localStorage.getItem('q-watched-v1') || '{}') || {};
      watched[(cfg.id || chip.getAttribute('data-ep-series') || 0) + '-' + season + '-' + episode] = Date.now();
      localStorage.setItem('q-watched-v1', JSON.stringify(watched));
