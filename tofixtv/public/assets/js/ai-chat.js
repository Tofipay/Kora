/* ALOKA Live — AI Assistant widget.
...
(function () {
  ...
  headTxt.appendChild(el('b', '', A.title || 'ALOKA Live AI'));
  ...
  function statusCopy(step) {
    if (isAr) {
      if (step === 'received') return 'تم استلام الرسالة، جارٍ البدء بالتحليل...';
      if (step === 'analyzing') return 'جارٍ التحليل...';
      if (step === 'searching') return 'جارٍ البحث في بيانات الموقع...';
      return 'جارٍ المعالجة...';
    }
    if (step === 'received') return 'Message received — starting analysis...';
    if (step === 'analyzing') return 'Analyzing...';
    if (step === 'searching') return 'Searching site data...';
    return 'Processing...';
  }
  ...
  function send(text) {
    ...
    statusBox = el('div', 'ai-msg ai-assistant ai-status-row');
    var statusBubble = el('div', 'ai-bubble ai-status-bubble', statusCopy('received'));
    statusBox.appendChild(statusBubble);
    body.appendChild(statusBox);
    scrollBottom();

    statusTimers.push(setTimeout(function () {
      if (statusBubble && statusBubble.parentNode) statusBubble.textContent = statusCopy('analyzing');
    }, 350));
    statusTimers.push(setTimeout(function () {
      if (statusBubble && statusBubble.parentNode) statusBubble.textContent = statusCopy('searching');
    }, 900));

    var typing = el('div', 'ai-msg ai-assistant ai-typing-row');
    var tb = el('div', 'ai-bubble ai-typing');
    tb.innerHTML = '<span class="ai-typing-text">' + (A.thinking || 'Analyzing and searching…') + '</span><i></i><i></i><i></i>';
    ...
    fetch('/api/ai-chat', {
      ...
      .then(function (res) {
        var delay = Math.max(0, 550 - (Date.now() - sentAt));
        setTimeout(function () {
          typing.remove();
          statusTimers.forEach(function (t) { clearTimeout(t); });
          statusTimers = [];
          if (statusBox && statusBox.parentNode) statusBox.remove();
          statusBox = null;
          var j = res.j || {};
