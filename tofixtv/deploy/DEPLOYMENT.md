...
1. Upload the whole project to your account (e.g. `~/aloka-live/`).
2. **Preferred:** point the domain's document root to `aloka-live/public`.
...
3. Make the storage folder writable:
   ...
   ```
4. Visit `https://aloka-code.shop/` — done. Old `*.php?id=…` URLs now 301 to
   the clean URLs automatically.
5. Open `https://aloka-code.shop/admin` — default password is in
...
```bash
sudo mkdir -p /var/www/aloka-live
sudo rsync -a ./ /var/www/aloka-live/
sudo chown -R www-data:www-data /var/www/aloka-live/storage /var/www/aloka-live/public/assets
sudo cp /var/www/aloka-live/deploy/nginx.conf /etc/nginx/sites-available/aloka-live
sudo ln -s /etc/nginx/sites-available/aloka-live /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx
```

Adjust the `fastcgi_pass` socket to your PHP-FPM version, then issue SSL:
`sudo certbot --nginx -d aloka-code.shop`.
...
* * * * *  php /path/to/aloka-live/deploy/notify-worker.php >/dev/null 2>&1

# Optional: pre-warm today's fixtures cache
*/5 * * * *  curl -s https://aloka-code.shop/api/live-scores >/dev/null
```
...
- [ ] Verify a legacy URL 301s: `curl -I https://aloka-code.shop/news.php?id=15`.
