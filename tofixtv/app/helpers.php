...
function match_article(array $m, array $state, array $channels = [], array $opts = []): array
{
    ...
    if ($isAr) {
        ...
        $p[] = "وتوفّر صفحة مباراة {$pair} على توفي إكس تيفي التشكيلة والإحصائيات وأحداث اللقاء وترتيب الفريقين في البطولة، إضافةً إلى البث المباشر بجودة عالية.";
        ...
        // 6) كيف تشاهد
        $howList = [];
        $howList[] = "افتح صفحة مباراة {$pair} على توفي إكس تيفي.";
...
function is_tofix_app(): bool
{
    ...
    // Exact UA (webview1.getSettings().setUserAgentString("com.tofixtv.app"))
    // or a WebView UA carrying the app identifier as a token.
    return $ua === 'com.tofixtv.app' || str_contains($ua, 'com.tofixtv.app');
}
