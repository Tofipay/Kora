...
function channel_group_url(array $category, array $group): string
{
    return path('channels/' . (string)($category['slug'] ?? '') . '/' . (string)($group['slug'] ?? ''));
}

function channel_play_url(array $channel): string
{
    return path('channels/play/' . (int)($channel['id'] ?? 0));
}

function catalog_image(?string $image, string $fallback = '/assets/img/channel.svg'): string
{
    $image = trim((string)$image);
    if ($image === '') return $fallback;
    if (preg_match('#^https?://#i', $image) || str_starts_with($image, '/assets/uploads/')) return $image;
    return $fallback;
}

function news_author(array $n): string
{
    foreach (['author_name', 'author', 'writer_name', 'writer', 'created_by'] as $key) {
        $value = $n[$key] ?? '';
        if (is_array($value)) $value = $value['name'] ?? '';
        if (is_string($value) && trim($value) !== '') return trim($value);
    }
    return \TofiXTv\Core\EditorialNews::defaultAuthor();
}

function news_category(array $n): string
{
    foreach (['category', 'category_name', 'section_name'] as $key) {
        $value = $n[$key] ?? '';
        if (is_array($value)) $value = $value['name'] ?? $value['title'] ?? '';
        if (is_string($value) && trim($value) !== '') return trim($value);
    }
    return '';
}
...
/* ============ Cinema (movies & series) URLs / images ============ */

/** /movie/{slug}-{id} for a TMDB movie payload (or plain id). */
function movie_url($movie, ?string $title = null): string
{
    if (is_array($movie)) {
        $id = (int)($movie['id'] ?? 0);
        $title = $title ?? (string)($movie['title'] ?? $movie['name'] ?? '');
    } else {
        $id = (int)$movie;
    }
    $slug = slugify((string)$title);
    return path('movie/' . ($slug !== '' ? $slug . '-' : '') . $id);
}

/** /series/{slug}-{id} for a TMDB tv payload (or plain id). */
function series_url($tv, ?string $title = null): string
{
    if (is_array($tv)) {
        $id = (int)($tv['id'] ?? 0);
        $title = $title ?? (string)($tv['name'] ?? $tv['title'] ?? '');
    } else {
        $id = (int)$tv;
    }
    $slug = slugify((string)$title);
    return path('series/' . ($slug !== '' ? $slug . '-' : '') . $id);
}

/** Detail URL for a mixed trending/search item (movie|tv). */
function cinema_url(array $item): string
{
    return \TofiXTv\Core\Tmdb::typeOf($item) === 'tv' ? series_url($item) : movie_url($item);
}

/** TMDB poster URL (w185/w342/w500) with brand fallback. */
function tmdb_poster(?string $file, string $size = 'w342'): string
{
    if (empty($file)) return '/assets/img/placeholder.svg';
    return TMDB_IMAGE_BASE . '/' . $size . $file;
}

/** TMDB backdrop URL (w780/w1280) with brand fallback. */
function tmdb_backdrop(?string $file, string $size = 'w1280'): string
{
    if (empty($file)) return '/assets/brand/og-default.png';
    return TMDB_IMAGE_BASE . '/' . $size . $file;
}

/** TMDB cast profile photo. */
function tmdb_profile(?string $file, string $size = 'w185'): string
{
    if (empty($file)) return '/assets/img/player.svg';
    return TMDB_IMAGE_BASE . '/' . $size . $file;
}
...
{
    if (is_array($nOrFile) && !empty($nOrFile['editorial_image'])) {
        $image = trim((string)$nOrFile['editorial_image']);
        if (preg_match('#^https?://#i', $image) || str_starts_with($image, '/assets/uploads/')) return $image;
    }
...
function live_clock(array $m): array
{
    ...
    if (in_array($status, [7, 8, 10, 11, 13], true)) {
    ...
    if ($start > 0) {
        ...
        $minute = (int)$raw;
    } elseif (!in_array($status, [1, 3, 5, 6, 9], true)) {
...
/**
 * Kickoff unix timestamp from whichever fields the payload carries.
 * match_timestamp is authoritative; match_date+match_time is the fallback
 * (some list payloads omit the timestamp).
 */
function match_kickoff_ts(array $m): int
{
    $ts = (int)($m['match_timestamp'] ?? 0);
    if ($ts > 1_000_000_000) return $ts;
    $d = trim((string)($m['match_date'] ?? ''));
    if ($d === '') return 0;
    $t = trim((string)($m['match_time'] ?? ''));
    return strtotime($d . ' ' . ($t !== '' ? $t : '00:00:00')) ?: 0;
}
 ...
 *
 * Upstream status codes (audited against live payloads + the type-100 period
 * markers, which share the same chronology):
 *   0 not started · 1 first half · 2 half-time · 3 second half · 4 finished ·
 *   5 ET first half · 6/9 ET second half · 7/8/13 penalty shootout ·
 *   10 end of extra time · 11 shootout start.
 *
 * The `live` flag is NOT reliable as an in-play signal (verified: matches in
 * extra time arrive with status=9, live=0), so the phase code is authoritative
 * and every in-play phase carries a time-based safety net: a match whose
 * kickoff is more than ~4h in the past can no longer be in play, no matter
 * what the feed says — it is reported as finished. This is what fixes the
 * "match stuck at 90′ / shown as not started / page refresh loop" bug: an
 * unknown phase code used to fall through to `upcoming`, which re-rendered the
 * kickoff countdown for a match already being played.
 */
function match_state(array $m): array
{
    $status  = (int)($m['status'] ?? 0);
    $live    = (int)($m['live'] ?? 0);
    $kick    = match_kickoff_ts($m);
    $elapsed = $kick > 0 ? time() - $kick : null;

    $finished = static fn(): array => [
        'key' => 'finished', 'label' => t('status.finished'),
        'live' => false, 'started' => true, 'clock' => null,
    ];
    $upcoming = static fn(): array => [
        'key' => 'upcoming', 'label' => format_time_12h($m['match_time'] ?? ''),
        'live' => false, 'started' => false, 'clock' => null,
    ];
    ...
    $overdue = $elapsed !== null && $elapsed > 4 * 3600;

    if (in_array($status, [1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 13], true)) {
        if ($overdue) return $finished();
        $clock = live_clock($m);
    ...
    // Unknown post-kickoff phase codes (the feed adds new ones): a match past
    // regulation length is finished; one inside it is treated as live rather
    // than "not started" (never render a countdown for a started match).
    if ($status > 4 && $elapsed !== null && $elapsed >= 0) {
        if ($elapsed > 110 * 60) return $finished();
        $clock = live_clock($m);
        return ['key' => 'live', 'label' => $clock['label'], 'live' => true, 'started' => true,
                'clock' => $clock, 'status' => $status];
    }
...
function match_article(array $m, array $state, array $channels = [], array $opts = []): array
{
    ...
    if ($isAr) {
        ...
        $p[] = "وتوفّر صفحة مباراة {$pair} على ALOKA Live التشكيلة والإحصائيات وأحداث اللقاء وترتيب الفريقين في البطولة، إضافةً إلى البث المباشر بجودة عالية.";
        ...
        // 6) كيف تشاهد
        $howList = [];
        $howList[] = "افتح صفحة مباراة {$pair} على ALOKA Live.";
...
function http_cache_validate(int $lastMod, string $etagSeed): void
{
    ...
    $etag = '"' . substr(sha1($etagSeed . '|' . $lastMod . '|' . \TofiXTv\Core\Lang::current()), 0, 20) . '"';
...
function site_logo(bool $dark = false): string
{
    ...
    $en = \TofiXTv\Core\Lang::current() === 'en' ? '-en' : '';
    return '/assets/brand/logo' . $en . ($dark ? '-dark' : '') . '.svg';
}

/**
 * Notification topics — every championship a visitor can subscribe to from
 * the notification bottom sheet, built DYNAMICALLY from the same matches API
 * that powers the fixtures (Api::allLeagues — every competition seen in a
 * -2…+7 day fixtures window, favourites pinned first, with logos).
 *
 * Slug format: "lg_{url_id}" — stable per championship, stored per-token and
 * matched by the Notifier against each match's championship at send time.
 *
 * @return array<int,array{slug:string,title:string,image:mixed}>
 */
function notify_topics(): array
{
    $out = [];
    foreach (\TofiXTv\Core\Api::allLeagues() as $lg) {
        $uid   = (int)($lg['url_id'] ?? 0);
        $title = trim((string)($lg['title'] ?? ''));
        if ($uid < 1 || $title === '') continue;
        $out[] = [
            'slug'  => 'lg_' . $uid,
            'title' => $title,
            'image' => $lg['image'] ?? null,
...
/* ==================== Android app (com.aloka.live.app) ==================== */

/**
 * True ONLY when the request comes from the official Android app, which
 * sends the exact User-Agent "com.aloka.live.app". Every app-specific feature
 * is gated on this — normal visitors/browsers are never affected.
 */
function is_tofix_app(): bool
{
    $ua = trim((string)($_SERVER['HTTP_USER_AGENT'] ?? ''));
    // Exact UA (webview1.getSettings().setUserAgentString("com.aloka.live.app"))
    // or a WebView UA carrying the app identifier as a token.
    return $ua === 'com.aloka.live.app' || str_contains($ua, 'com.aloka.live.app');
}

/**
 * Android intent:// deep link that opens a stored stream URL in the
 * official app (scheme xmtv, package com.aloka.live.app):
 *   intent://{stored URL}#Intent;scheme=xmtv;package=com.aloka.live.app;end
 */
function app_intent_url(string $url): string
{
    return 'intent://' . $url . '#Intent;scheme=xmtv;package=com.aloka.live.app;end';
}
