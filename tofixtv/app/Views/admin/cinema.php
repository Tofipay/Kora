...
.fchips a.on{background:linear-gradient(135deg,rgba(76, 14, 205,.18),rgba(124, 77, 255,.10));color:var(--p);border-color:rgba(76, 14, 205,.4)}
...
.cine-row img{width:40px;height:60px;object-fit:cover;border-radius:8px;background:#10033d;flex:none}
.cine-row .noimg{width:40px;height:60px;border-radius:8px;background:#10033d;flex:none;display:grid;place-items:center;color:var(--tx3);font-size:10px}
...
.cine-row .download-field{width:min(260px,100%);min-width:210px;padding:8px 10px;font-size:12px;direction:ltr}
.cchip{font-size:11px;font-weight:800;padding:4px 10px;border-radius:999px;white-space:nowrap}
.c-on{background:rgba(76, 14, 205,.15);color:var(--p)}
.c-app{background:rgba(124, 77, 255,.15);color:var(--p2)}
.c-off{background:rgba(239,68,68,.15);color:#f87171}
.c-download{background:rgba(124, 77, 255,.14);color:var(--p2)}
.rchip{font-size:11px;font-weight:800;padding:4px 9px;border-radius:7px;background:#10033d;color:var(--tx2)}
.rchip.r-18{background:rgba(239,68,68,.18);color:#f87171}
.rchip.r-16{background:rgba(245,158,11,.18);color:#fbbf24}
.bulkbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;background:#10033d;border:1px solid var(--line);border-radius:12px;padding:10px 14px;margin-bottom:12px}
...
<div class="card">
  ...
  <small class="hint">«التطبيق فقط» يبقي كل الصفحات والوصف والبيانات المهيكلة كما هي للأرشفة، لكنه يخفي مشغّل الفيديو على الموقع ويعرض شاشة قفل مع رابط تحميل التطبيق. المشغّل يعمل طبيعياً داخل التطبيق (User-Agent: com.aloka.live.app).</small>
...
<div class="card">
  ...
  <form method="post" action="<?= e($self) ?>" id="bulk-form">
    ...
    <div class="cine-row">
      ...
      <?php if ($r['download_url'] !== ''): ?><span class="cchip c-download">تنزيل متاح</span><?php endif; ?>
    </div>
    <?php endforeach; ?>
  </form>
  <?php endif; ?>
...
<div class="card">
  ...
  <div class="cine-row">
    ...
    <form method="post" action="<?= e($self) ?><?= $q !== '' ? '?q=' . rawurlencode($q) : '' ?>">
      ...
      <input class="download-field" type="url" name="download_url" value="<?= e($r['download_url']) ?>" placeholder="رابط التنزيل الاختياري https://..." aria-label="رابط التنزيل الاختياري">
