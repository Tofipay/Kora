 ...
 * Design language: glassy dark surface, ALOKA purple accents, responsive
...
function admin_top(string $title, string $active = ''): void
{
    $nav = [
        ...
        'channel-catalog'=> ['channel-catalog','قسم القنوات',     'M3 6h18v12H3zM8 6v12M16 6v12M9 3l3 3 3-3'],
        'editorial-news'=> ['editorial-news','إدارة الأخبار',    'M3 4h18v16H3zM7 9h10M7 13h7M7 17h5'],
...
<title><?= e($title) ?> · لوحة تحكم ALOKA Live</title>
...
<style>
:root{
  --p:#4C0ECD;--p2:#7C4DFF;--ink:#10033D;--bg:#10033D;--card:#1B0761;--card2:#241068;
...
body{background:
  radial-gradient(700px 380px at 90% -5%,rgba(124, 77, 255,.10),transparent 70%),
  radial-gradient(760px 420px at 5% 0%,rgba(76, 14, 205,.12),transparent 70%),
  var(--bg);
...
aside{
  ...
  background:rgba(27, 7, 97,.72);-webkit-backdrop-filter:blur(18px);backdrop-filter:blur(18px);
...
aside .brand .dot{width:30px;height:30px;border-radius:9px;flex:none;display:grid;place-items:center;
  background:linear-gradient(135deg,var(--p),var(--p2));color:#ffffff}
...
aside a.nav.on{background:linear-gradient(135deg,rgba(76, 14, 205,.18),rgba(124, 77, 255,.10));color:var(--p)}
...
/* Cards & primitives */
.card{background:rgba(27, 7, 97,.7);border:1px solid var(--line);border-radius:var(--r);padding:20px;margin-bottom:16px;box-shadow:var(--shadow)}
...
input[type=text],input[type=password],input[type=email],input[type=url],input[type=number],input[type=datetime-local],input[type=file],textarea,select{
  width:100%;background:#10033d;border:1px solid var(--line);border-radius:11px;padding:10px 13px;color:var(--tx);font:inherit}
input:focus,textarea:focus,select:focus{outline:2px solid rgba(76, 14, 205,.5);outline-offset:1px;border-color:transparent}
...
.btn{display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,var(--p),color-mix(in srgb,var(--p) 70%,var(--p2)));
  color:#ffffff;font-weight:800;padding:10px 22px;border-radius:999px;border:0;cursor:pointer;font:inherit;margin-top:14px;transition:.15s}
.btn:hover{transform:translateY(-1px)}
.btn:active{transform:scale(.97)}
.btn.ghost{background:transparent;border:1.5px solid var(--line);color:var(--tx)}
.btn.danger{background:var(--danger);color:#fff}
.msg{background:rgba(76, 14, 205,.12);border:1px solid rgba(76, 14, 205,.4);color:var(--p);padding:11px 16px;border-radius:12px;margin-bottom:16px;font-weight:700}

/* Stat cards */
.stat-grid{display:grid;gap:14px;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));margin-bottom:16px}
.stat{background:rgba(27, 7, 97,.7);border:1px solid var(--line);border-radius:var(--r);padding:16px 18px;position:relative;overflow:hidden}
...
.stat .pulse{width:8px;height:8px;border-radius:50%;background:var(--p);box-shadow:0 0 0 0 rgba(76, 14, 205,.5);animation:pl 1.6s infinite}
@keyframes pl{0%{box-shadow:0 0 0 0 rgba(76, 14, 205,.45)}70%{box-shadow:0 0 0 8px rgba(76, 14, 205,0)}100%{box-shadow:0 0 0 0 rgba(76, 14, 205,0)}}
...
.bar div{flex:1;background:linear-gradient(180deg,var(--p),#7c4dff);border-radius:5px 5px 0 0;min-height:4px;position:relative;transition:height .5s cubic-bezier(.2,.7,.3,1)}
...
.metric .mrow .track{flex:1 1 auto;min-width:0;height:10px;border-radius:999px;background:#10033d;overflow:hidden}
.metric .mrow .fill{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,var(--p2),var(--p));box-shadow:0 0 10px rgba(76, 14, 205,.35);transition:width .8s cubic-bezier(.2,.7,.3,1)}
...
.rank .n{width:24px;height:24px;flex:none;border-radius:8px;background:#10033d;display:grid;place-items:center;font-weight:800;font-size:12px;color:var(--tx2)}
...
.update-hero{display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:space-between;
  background:linear-gradient(120deg,rgba(76, 14, 205,.14),rgba(124, 77, 255,.08));border:1px solid rgba(76, 14, 205,.3)}
...
.sec-item{display:flex;align-items:center;gap:10px;background:#10033d;border:1px solid var(--line);border-radius:11px;padding:10px 14px;margin-bottom:6px;cursor:grab}
...
.kpi{background:rgba(27, 7, 97,.7);border:1px solid var(--line);border-radius:var(--r);padding:18px;text-align:center}
...
/* Mobile top bar + drawer navigation (hidden on desktop) */
.admin-topbar{display:none}
.drawer-overlay{display:none}
@media(max-width:860px){
  body{grid-template-columns:1fr}
  .admin-topbar{position:sticky;top:0;z-index:40;display:flex;align-items:center;gap:12px;
    padding:12px 16px;background:rgba(16, 3, 61,.85);-webkit-backdrop-filter:blur(16px);backdrop-filter:blur(16px);
    border-bottom:1px solid var(--line)}
  .admin-topbar .tb-title{font-weight:900;font-size:15px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .admin-topbar .tb-burger{width:42px;height:42px;border-radius:12px;border:1px solid var(--line);
    background:var(--card);color:var(--tx);display:grid;place-items:center;cursor:pointer;flex:none;
    -webkit-tap-highlight-color:transparent}
  aside{position:fixed;inset-block:0;inset-inline-start:0;width:min(300px,84vw);height:100dvh;z-index:60;
    transform:translateX(100%);transition:transform .28s cubic-bezier(.2,.7,.3,1);
    background:rgba(20, 6, 60,.97);border-inline-end:1px solid var(--line);box-shadow:var(--shadow)}
  [dir=ltr] aside{transform:translateX(-100%)}
  body.drawer-open aside{transform:translateX(0)}
  aside a.nav{padding:13px 14px;font-size:14.5px}
  .drawer-overlay{display:block;position:fixed;inset:0;z-index:50;background:rgba(10, 2, 38,.6);
...
<header class="admin-topbar">
  <button class="tb-burger" type="button" aria-label="القائمة" onclick="document.body.classList.toggle('drawer-open')">
    <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
  </button>
  <span class="tb-title"><?= e($title) ?></span>
  <span class="dot" style="width:30px;height:30px;border-radius:9px;display:grid;place-items:center;background:linear-gradient(135deg,var(--p),var(--p2));color:#ffffff">
    <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M4 13l4-4 4 4 8-8"/></svg>
  </span>
</header>
<div class="drawer-overlay" onclick="document.body.classList.remove('drawer-open')"></div>
<aside>
  <div class="brand"><span class="dot"><svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M4 13l4-4 4 4 8-8"/></svg></span> ALOKA Live</div>
