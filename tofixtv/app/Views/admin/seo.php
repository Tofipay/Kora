...
<div class="card">
  <form method="post">
    ...
    <div class="row">
      <div><label>Home title (Arabic)</label><input type="text" name="title_ar" value="<?= e($s['title_ar'] ?? '') ?>" placeholder="توفي إكس تيفي — نتائج مباشرة…"></div>
      <div><label>Home title (English)</label><input type="text" name="title_en" value="<?= e($s['title_en'] ?? '') ?>" placeholder="ToFi X Tv — Live scores…"></div>
    </div>
