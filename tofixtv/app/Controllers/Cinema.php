...
final class Cinema
{
    ...
    public static function show(string $slug): void
    {
        ...
        $seasonNumbers = array_map(static fn($s) => (int)($s['season_number'] ?? 0), $seasons);
        if ($seasonNumbers && !in_array($curSeason, $seasonNumbers, true)) $curSeason = $seasonNumbers[0];
        // The method already existed; the UI now uses its real episode stills,
        // titles and runtimes. It is cached by Tmdb and only the selected
        // season is requested, so no extra catalogue-sized payload is loaded.
        $seasonData = Tmdb::tvSeason($id, $curSeason);
        $episodes = array_values(array_filter($seasonData['episodes'] ?? [], 'is_array'));
        $curEpisode = max(1, query_int('episode', 1));
        if ($episodes) {
            $episodeNumbers = array_map(static fn($ep) => (int)($ep['episode_number'] ?? 0), $episodes);
            if (!in_array($curEpisode, $episodeNumbers, true)) $curEpisode = max(1, $episodeNumbers[0]);
        }
