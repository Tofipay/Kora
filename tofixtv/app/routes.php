...
use TofiXTv\Controllers\{
    ...
    Standings, Scorers, StaticPages, Sitemap, Media, Admin, ApiJson, Videos, Channels
};
...
/* ---------- Standalone channel catalogue (does not replace match channels) ---------- */
$router->get('/channels', [Channels::class, 'index']);
$router->get('/channels/play/{id:\d+}', fn($a) => Channels::play((int)$a['id']));
$router->get('/channels/{category}/{group}', fn($a) => Channels::group($a['category'], $a['group']));
