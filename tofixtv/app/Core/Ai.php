...
final class Ai
{
    ...
    {
        $lang = in_array($hint, ['ar', 'en'], true)
            ? $hint
            : (preg_match('#^/en(/|$)#', (string)($page['path'] ?? '')) ? 'en' : 'ar');
        if (preg_match('/\p{Arabic}/u', $message)) return 'ar';
        if (preg_match_all('/[a-zA-Z]{2,}/', $message) >= 2) return 'en';
        return $lang;
    }
    ...
    private static function t(string $key): string
    {
        ...
        $map = [
            ...
            'no_match_found'  => [$ar ? 'لم أجد هذه المباراة في بيانات الموقع (لا في السجل ولا في الجدول القادم). تأكد من أسماء الفريقين أو التاريخ.' : "I couldn't find that match in the site data (neither past nor scheduled). Check the team names or the date."],
