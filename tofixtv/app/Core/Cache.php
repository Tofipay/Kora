...
final class Cache
{
    ...
    /** Age of the stored payload in seconds, or null when absent. */
    public static function age(string $key): ?int
    {
        $file = self::path($key);
        if (!is_file($file)) return null;
        return max(0, time() - (int)filemtime($file));
    }

    /**
     * Best-effort refresh lock (stampede/duplicate-request guard): returns
     * true for exactly one caller per $ttl window, so N concurrent visitors
     * trigger ONE upstream revalidation instead of N identical requests.
     */
    public static function lock(string $key, int $ttl = 55): bool
    {
        if (!is_dir(CACHE_DIR)) @mkdir(CACHE_DIR, 0755, true);
        $file = CACHE_DIR . '/' . md5('lock|' . $key) . '.lock';
    ...
    public static function flush(): int
    {
        ...
        foreach (glob(CACHE_DIR . '/*.lock') ?: [] as $f) @unlink($f);
        return $n;
    }
