...
<section class="watch-page" dir="<?= Lang::dir() ?>">
  <div class="container watch-top">
    <a class="wt-back" href="<?= e(path('yacine/' . $id)) ?>">
      <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
      <?= e($ar ? 'السيرفرات' : 'Servers') ?>
    </a>
  </div>
  ...
  <div class="container">
    <div class="player-shell glass" id="qplayer"
         ...
         data-refresh="<?= e(path('yacine/' . $id . '/src')) ?>"
         data-ttl="<?= (int)($ttl ?? 0) ?>"
      ...
      <div class="pl-stage">
        ...
        <div class="pl-controls" data-controls>
          ...
          <div class="pl-bar">
            ...
            <button class="pl-btn" data-aspect aria-label="aspect ratio" title="<?= e($ar ? 'نسبة العرض' : 'Aspect ratio') ?>">
      ...
      <div class="pl-servers" data-servers-bar>
        <span class="pls-label"><?= e($ar ? 'السيرفرات' : 'Servers') ?></span>
        <?php foreach ($sources as $i => $s): ?>
          <button class="pls-btn<?= $i === 0 ? ' on' : '' ?>" data-server="<?= (int)$i ?>">
            <span class="pls-dot"></span><?= e($s['label'] ?? ('S' . ($i + 1))) ?>
          </button>
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
