...
final class Home
{
    public static function index(): void
    {
        ...
        // --- Cinema rows (fail-soft: empty rows simply don't render) ---
        $movies      = array_slice(\TofiXTv\Core\Tmdb::popularMovies()['results'] ?? [], 0, 12);
        $seriesRow   = array_slice(\TofiXTv\Core\Tmdb::popularTv()['results'] ?? [], 0, 12);
        $trendingAll = array_slice(\TofiXTv\Core\Tmdb::trending('all', 'week')['results'] ?? [], 0, 12);
        $newContent  = array_slice(array_merge(
            array_slice(\TofiXTv\Core\Tmdb::nowPlayingMovies()['results'] ?? [], 0, 6),
            array_slice(\TofiXTv\Core\Tmdb::airingTodayTv()['results'] ?? [], 0, 6)
        ), 0, 12);
