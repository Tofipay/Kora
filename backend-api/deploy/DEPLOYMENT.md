...
1. Upload the whole project to your account (e.g. `~/qamhad-live/`).
2. **Preferred:** point the domain's document root to `qamhad-live/public`.
...
3. Make the storage folder writable:
   ...
   ```
4. Visit `https://live.qamhad.com/` — done. Old `*.php?id=…` URLs now 301 to
   the clean URLs automatically.
5. Open `https://live.qamhad.com/admin` — default password is in
...
```bash
sudo mkdir -p /var/www/qamhad-live
sudo rsync -a ./ /var/www/qamhad-live/
sudo chown -R www-data:www-data /var/www/qamhad-live/storage /var/www/qamhad-live/public/assets
sudo cp /var/www/qamhad-live/deploy/nginx.conf /etc/nginx/sites-available/qamhad-live
sudo ln -s /etc/nginx/sites-available/qamhad-live /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx
```

Adjust the `fastcgi_pass` socket to your PHP-FPM version, then issue SSL:
`sudo certbot --nginx -d live.qamhad.com`.
...
* * * * *  php /path/to/qamhad-live/deploy/notify-worker.php >/dev/null 2>&1

# Optional: pre-warm today's fixtures cache
*/5 * * * *  curl -s https://live.qamhad.com/api/live-scores >/dev/null
```
...
- [ ] Verify a legacy URL 301s: `curl -I https://live.qamhad.com/news.php?id=15`.
