...
final class Api
{
    ...
    /** Raw GET with cache + stale fallback. Returns decoded array or []. */
    public static function get(string $url, int $ttl = CACHE_TTL_MATCHES): array
    {
        ...
        // Negative cache: after an upstream failure, back off for 60s
        // instead of blocking every page view on fresh timeouts.
    ...
    {
        $map = \Qamhad\Core\Settings::get('league_images', []);
    ...
    public static function allLeagues(): array
    {
        ...
        if ($mapDirty) \Qamhad\Core\Settings::set('league_images', $imgMap);
    ...
    private static function cleanNews($items): array
    {
        ...
        return array_values(array_filter($items, fn($it) =>
    ...
    public static function allNewsPage(): array
    {
        ...
        $main      = is_array($d['main'] ?? null) && !empty($d['main']['id']) && !is_blocked_text((string)($d['main']['title'] ?? '')) ? $d['main'] : null;
