...
@Override
	public void onStart() {
		...
		if (getDialog() == null||getDialog().getWindow() == null) {
				...
				if (dialog != null) {
						float width = getDP(getActivity(),280);
						int height = ViewGroup.LayoutParams.WRAP_CONTENT;
						dialog.getWindow().setLayout((int)width, height);
  ...
  public static final class TrackSelectionViewFragment extends Fragment
    ...
    for (int i = 0; i < trackSelectionView.getChildCount(); i++) {
        int color_for_state_checked = Color.parseColor("#ffffff");
        int color_for_state_normal = Color.parseColor("#ffffff");
        View child = trackSelectionView.getChildAt(i);
     
        if (child instanceof CheckedTextView) {
            int [][] states = {{android.R.attr.state_checked}, {}};
int [] colors = {color_for_state_checked, color_for_state_normal};
           if(Build.VERSION.SDK_INT>22){
            ((CheckedTextView) child).setCheckMarkTintList(new ColorStateList(states, colors));
            }else{
            ((CheckedTextView) child).setCheckMarkDrawable(R.drawable.checkmark_selector);
            }
            ((CheckedTextView) child).setTextColor(Color.parseColor("#ffffff"));
            }
        }
    } 
  ...
  public static float getDP(Context context,int toDP ){
        if (toDP == 0){
            return 0;
        } else{
     
            return TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, toDP, context.getResources().getDisplayMetrics());
